...
from mypy.nodes import MemberExpr, StrExpr, Var
...
# positions of SafePrompt constructor arguments
TEMPLATE_ARG_INDEX = 0
MODEL_ARG_INDEX = 1
TEMPLATE_TYPE_ARG_INDEX = 2


def safe_prompt_init_hook(ctx: FunctionContext) -> Type:
    ...
    if len(ctx.args) > TEMPLATE_TYPE_ARG_INDEX and ctx.args[TEMPLATE_TYPE_ARG_INDEX]:
        kind = ctx.args[TEMPLATE_TYPE_ARG_INDEX][0]
    ...
    if len(ctx.arg_types) <= MODEL_ARG_INDEX or not ctx.arg_types[MODEL_ARG_INDEX]:
        return ctx.default_return_type
    model_arg = ctx.arg_types[MODEL_ARG_INDEX][0]
    ...
    if isinstance(model_arg, TypeType):
        ...
        ctx.api.fail("Second argument to SafePrompt must be a BaseModel class", tpl_expr)
        return ctx.default_return_type

    # 5) Gather its declared fields
    fields = {name for name, symnode in model_info.names.items() if isinstance(symnode.node, Var)}
...
def plugin(version: str) -> type[SafePromptPlugin]:
